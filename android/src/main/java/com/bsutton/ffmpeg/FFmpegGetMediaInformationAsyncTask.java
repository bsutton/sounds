 ...
 * @since 0.1.0
 */
public class FFmpegGetMediaInformationAsyncTask extends AsyncTask<String, Integer, MediaInformation> {

    private final String path;
    private final MethodChannel.Result result;
    private final FFmpegResultHandler FFmpegResultHandler;

    FFmpegGetMediaInformationAsyncTask(final String path, final FFmpegResultHandler FFmpegResultHandler, final MethodChannel.Result result) {
        this.path = path;
        this.result = result;
    ...
    protected MediaInformation doInBackground(final String... unusedArgs) {
        Log.d(FFmpegPlugin.LIBRARY_NAME, String.format("Getting media information for %s.", path));
        return FFprobe.getMediaInformation(path);
    }

    @Override
    protected void onPostExecute(final MediaInformation mediaInformation) {
        FFmpegResultHandler.success(result, FFmpegPlugin.toMediaInformationMap(mediaInformation));
    }
