 ...
 * @since 0.2.9
 */
public class FFmpegExecuteFFprobeAsyncArgumentsTask extends AsyncTask<String, Integer, Integer> {

    private final MethodChannel.Result result;
    private final List<String> arguments;
    private final FFmpegResultHandler FFmpegResultHandler;

    FFmpegExecuteFFprobeAsyncArgumentsTask(final List<String> arguments, final FFmpegResultHandler FFmpegResultHandler, final MethodChannel.Result result) {
    ...
    protected Integer doInBackground(final String... unusedArgs) {
        ...
        Log.d(FFmpegPlugin.LIBRARY_NAME, String.format("Running FFprobe with arguments: %s.", Arrays.toString(argumentsArray)));

        int rc = FFprobe.execute(argumentsArray);

        Log.d(FFmpegPlugin.LIBRARY_NAME, String.format("FFprobe exited with rc: %d", rc));
