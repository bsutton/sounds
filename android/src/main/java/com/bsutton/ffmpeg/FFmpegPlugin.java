 ...
 * @since 0.1.0
 */
public class FFmpegPlugin implements MethodCallHandler, EventChannel.StreamHandler {
    ...
    static FFmpegPlugin FFmpegPlugin; // singleton
    static Context              androidContext;


    public static void attachFFmpegPlugin( Context ctx, BinaryMessenger messenger )
    {
        assert ( FFmpegPlugin == null );
        FFmpegPlugin = new FFmpegPlugin();
    ...
    public static void registerWith(final Registrar registrar) {
        FFmpegPlugin FFmpegPlugin = new FFmpegPlugin(registrar);
...
*/
    private FFmpegPlugin() {


        this.FFmpegResultHandler = new FFmpegResultHandler();
    }
    ...
    public void onMethodCall(final MethodCall call, final Result result) {
        if (call.method.equals("getPlatform")) {
            ...
            FFmpegResultHandler.success(result, toStringMap(KEY_PLATFORM, PLATFORM_NAME + "-" + abi));

        } else if (call.method.equals("getFFmpegVersion")) {

            final String version = Config.getFFmpegVersion();
            FFmpegResultHandler.success(result, toStringMap(KEY_VERSION, version));

        } else if (call.method.equals("executeFFmpegWithArguments")) {

            List<String> arguments = call.argument("arguments");

            final FFmpegExecuteFFmpegAsyncArgumentsTask asyncTask = new FFmpegExecuteFFmpegAsyncArgumentsTask(arguments, FFmpegResultHandler, result);
            ...
            final FFmpegExecuteFFprobeAsyncArgumentsTask asyncTask = new FFmpegExecuteFFprobeAsyncArgumentsTask(arguments, FFmpegResultHandler, result);
            asyncTask.execute("dummy-trigger");
            ...
            FFmpegResultHandler.success(result, toIntMap(KEY_LOG_LEVEL, levelToInt(level)));
            ...
            FFmpegResultHandler.success(result, toMap(Config.getLastReceivedStatistics()));
            ...
            final FFmpegGetMediaInformationAsyncTask asyncTask = new FFmpegGetMediaInformationAsyncTask(path, FFmpegResultHandler, result);
            asyncTask.execute();

        } else if (call.method.equals("registerNewFFmpegPipe")) {

            final String pipe = Config.registerNewFFmpegPipe(getActiveContext());
            FFmpegResultHandler.success(result, toStringMap(KEY_PIPE, pipe));

        } else {
            FFmpegResultHandler.notImplemented(result);
        }
    }
    ...
    protected void emitLogMessage(final LogMessage logMessage) {
        ...
        FFmpegResultHandler.success(eventSink, logWrapperMap);
    }
    ...
    protected void emitStatistics(final Statistics statistics) {
        ...
        FFmpegResultHandler.success(eventSink, statisticsMap);
    }
