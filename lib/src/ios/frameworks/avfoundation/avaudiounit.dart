...
class AVAudioUnit extends AVAudioNode {
  ...
  AudioComponentDescription get audioComponentDescription {
    Pointer<Void> result =
        perform(SEL('audioComponentDescription'), decodeRetVal: false);
    return AudioComponentDescription.fromPointer(result);
  }

  set audioComponentDescription(
          AudioComponentDescription audioComponentDescription) =>
      perform(SEL('setAudioComponentDescription:'),
          args: [audioComponentDescription]);
  ...
  set audioUnit(AudioUnit audioUnit) =>
      perform(SEL('setAudioUnit:'), args: [audioUnit]);
  ...
  set AUAudioUnit(AUAudioUnit AUAudioUnit) =>
      perform(SEL('setAUAudioUnit:'), args: [AUAudioUnit]);
  ...
  String get manufacturerName {
    Pointer<Void> result =
        perform(SEL('manufacturerName'), decodeRetVal: false);
    return NSString.fromPointer(result).raw;
  }

  set manufacturerName(String manufacturerName) =>
      perform(SEL('setManufacturerName:'), args: [manufacturerName]);

  NSUInteger get version {
    return perform(SEL('version'));
  }

  set version(NSUInteger version) =>
      perform(SEL('setVersion:'), args: [version]);
  @NativeAvailable(macos: '10.11', ios: '9.0', tvos: '9.0')
  static void instantiateWithComponentDescriptionOptionsCompletionHandler(
      AudioComponentDescription audioComponentDescription,
      AudioComponentInstantiationOptions options,
      {void completionHandler(AVAudioUnit audioUnit, NSError error)}) {
    Class('AVAudioUnit').perform(
        SEL('instantiateWithComponentDescription:options:completionHandler:'),
        args: [audioComponentDescription, options, completionHandler]);
  }

  bool loadAudioUnitPresetAtURLError(NSURL url, NSObjectRef<NSError> outError) {
    return perform(SEL('loadAudioUnitPresetAtURL:error:'),
        args: [url, outError]);
  }
}
