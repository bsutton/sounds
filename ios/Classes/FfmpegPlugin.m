...
{
        [FFmpegPlugin registerWithRegistrar: registrar];
}
...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    if ([@"getPlatform" isEqualToString:call.method]) {
        ...
        result([FFmpegPlugin toStringDictionary:KEY_PLATFORM :[NSString stringWithFormat:@"%@-%@", PLATFORM_NAME, architecture]]);

    } else if ([@"getFFmpegVersion" isEqualToString:call.method]) {

        result([FFmpegPlugin toStringDictionary:KEY_VERSION :[MobileFFmpegConfig getFFmpegVersion]]);
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            result([FFmpegPlugin toIntDictionary:KEY_RC :[NSNumber numberWithInt:rc]]);
        });
        ...
        result([FFmpegPlugin toIntDictionary:KEY_LOG_LEVEL :[NSNumber numberWithInt:logLevel]]);
        ...
        result([FFmpegPlugin toStatisticsDictionary:statistics]);
        ...
        result([FFmpegPlugin toStringDictionary:KEY_PACKAGE_NAME :packageName]);
        ...
        result([FFmpegPlugin toIntDictionary:KEY_LAST_RC :[NSNumber numberWithInt:lastReturnCode]]);
        ...
        result([FFmpegPlugin toStringDictionary:KEY_LAST_COMMAND_OUTPUT :lastCommandOutput]);
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            result([FFmpegPlugin toMediaInformationDictionary:mediaInformation]);
        });

    } else if ([@"registerNewFFmpegPipe" isEqualToString:call.method]) {

        NSString *pipe = [MobileFFmpegConfig registerNewFFmpegPipe];
        result([FFmpegPlugin toStringDictionary:KEY_PIPE :pipe]);
...
- (void)emitLogMessage:(NSDictionary*)logMessage{
    _eventSink([FFmpegPlugin toStringDictionary:EVENT_LOG withDictionary:logMessage]);
}

- (void)emitStatistics:(Statistics*)statistics{
    NSDictionary *dictionary = [FFmpegPlugin toStatisticsDictionary:statistics];
    _eventSink([FFmpegPlugin toStringDictionary:EVENT_STAT withDictionary:dictionary]);
}
    ...
    if (mediaInformation != nil) {
        ...
        if (streams != nil && ([streams count] > 0)) {
            ...
            for (int i=0; i < [streams count]; i++) {
                ...
                [array addObject: [FFmpegPlugin toStreamInformationDictionary:streamInformation]];
            }
